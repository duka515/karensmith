(function () {
  ...
  function openOffer(url, kind) {
    ...
    var kicker = document.getElementById("popupKicker"); if (kicker) kicker.textContent = kickers[kind] || "Exclusive access";
    if (!overlay) return;
    ...
    document.documentElement.classList.add("funnel-open"); document.body.classList.add("funnel-open");
  }
  function bind(selector, url, kind) {
    document.querySelectorAll(selector).forEach(function (el) {
      el.removeAttribute("href");
      el.addEventListener("click", function (e) { e.preventDefault(); e.stopPropagation(); openOffer(url, kind); });
    });
  }
